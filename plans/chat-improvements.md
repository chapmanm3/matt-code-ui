...
- **UI**: Session sidebar with all sessions (`#session-sidebar`)
- **Data**: Uses `SessionInfo` type (id, title, time)
- **Files**: `src/main.ts` - `fetchSessions()`, `switchToSession()`, `loadSessions()`
- **Completed**: 2026-05-04

### 1.2 Session Titles ✅
- **Current**: Uses `Session.title` from API response
- **UI**: Displays title in session list and tab bar
- **Fallback**: Falls back to "Untitled" if no title provided
- **Completed**: 2026-05-04

### 1.3 Delete Sessions ✅
- **API**: `DELETE /session/:id`
- **UI**: Delete button (×) on each session list item
- **Confirm**: Confirmation dialog before deleting
- **Completed**: 2026-05-04
...
- **UI**: Dropdown selector in chat header with optgroups for each provider
- **State**: Stored in `providers` and `defaultModels` variables
- **Files**: `src/main.ts` - `fetchProviders()`, `populateModelSelector()`
- **Note**: API returns `models` as object (key-value map), not array
- **Completed**: 2026-05-04

### 2.2 Display Current Model ✅
- **UI**: Shows in status bar as `[providerID/modelID]`
- **Update**: Model selector reflects current selection
- **Files**: `src/main.ts` - `updateStatusBar()`, `updateModelSelector()`
- **Completed**: 2026-05-04

### 2.3 Per-Message Model Override ✅
- **API**: Pass `model: { providerID, modelID }` in `POST /session/:id/message` body
- **UI**: Model selector dropdown allows changing model before sending
- **Files**: `src/main.ts` - `sendChatMessage()` includes `currentModel` in body
- **Completed**: 2026-05-04
...
### 3.1 Message History ✅
- **API**: `GET /session/:id/message` to list messages
- **Use case**: Loads previous messages when switching sessions
- **State**: Messages stored per session in `ChatTab.messages`
- **Completed**: 2026-05-04
...
3. **Delete sessions** (Section 1.3)
4. **Model selection dropdown** (Section 2.1, 2.2, 2.3)
5. **Message history loading** (Section 3.1)

### ⏳ Pending (High Priority)
6. **Markdown syntax highlighting** (Section 6.1)
7. **File search integration** (Section 4.1)

### ⏳ Pending (Medium Priority)
8. **Resend/retry messages** (Section 3.2)
9. **Undo/redo buttons** (Section 3.3)
10. **Slash commands** (Section 7.1)
11. **Typing indicators** (Section 6.3)
12. **Agent selection** (Section 5.1)

### ⏳ Pending (Low Priority / Advanced)
13. Fork sessions (Section 1.4)
14. Message summarization (Section 3.4)
15. Symbol search (Section 4.2)
16. File diffs (Section 4.3)
17. Open files in editor (Section 4.4)
18. Structured output (Section 10.1)
19. Tool usage display (Section 10.2)
20. Session sharing (Section 8)
21. Permission handling (Section 10.3)
22. Project switching (Section 9)
23. LSP integration (Section 10.4)
...
- Note: `@opencode-ai/sdk` is installed but not currently used

### State Management
- ✅ Session cache: Messages stored in `ChatTab.messages`
- ✅ Current model preference: `currentModel` variable
...
- ✅ `src/main.ts` - Added model selection, providers fetch, updated chat logic
- ✅ `src/styles.css` - Added `.chat-header`, `.model-selector` styles
- ✅ `index.html` - Added model selector dropdown in chat header
